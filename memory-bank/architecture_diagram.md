...
```mermaid
graph TD
    A[Telegram User] -->|"Clicks Buy button / Sends buy command"| B(Telegram Bot Handler)
    B -->|Initiates conversation, collects parameters| C(Hummingbot Integration Module)
    C -->|Creates config, deploys bot via API| D[Hummingbot API Server]
    D -->|Manages Hummingbot Instances| E[Hummingbot Docker Containers]
    C -->|Stores active trade info| F["Active Trades Persistent Storage (active_trades.json)"]
    F -->|Periodically read by| G(Bot Status Monitor)
    G -->|Checks bot status via API| D
    D -->|Returns status| G
    G -->|If bot stopped, archives bot via API| D
    G -->|Updates active trades storage| F
    G -->|Sends completion notification| H(Telegram Alerts Module)
    H -->|Sends message to| A
    H -->|Sends volume alerts with Buy button| A
```
...
graph TD
    ...
    I --> J["Continue to next active trade / End loop iteratio"]
