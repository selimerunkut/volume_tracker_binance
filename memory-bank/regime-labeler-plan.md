...
- The labeler runs only after the Freqtrade downloader has completed. Before and after reading, verify file metadata/hash is stable; reject a file that changes during processing. The source may be up to seven days behind the current UTC date, but must contain a complete day no older than seven days; the latest completed day is the explicit as-of date shown to users. A partial current day is discarded.
